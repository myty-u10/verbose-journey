...
N2 = int(input('サブネットの故障とみなすタイムアウトの回数を入力してください : '))      

while True:
    file = input('サブネットのログファイルのパスを入力してください(fで終了) : ')
    if file == 'f':
        break
    ...
    switch = []
    judge = 0
    minimum, maximum = 100000000000000, 0
    ...
    if n2 > N2:
        print('このサブネットは故障しています。')
        print('サブネットの故障期間 : ',end='')
        e = Time(maximum) - Time(minimum)
        e.print_time()
        print()
